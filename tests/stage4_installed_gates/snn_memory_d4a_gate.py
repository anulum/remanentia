...
FIXTURE_ENCODER = EncoderConfig(
    feature_dim=16, packet_ms=5, silent_ms=2, active_fraction=0.1, projection_seed=1729
)
...
def _git(repo: Path, *arguments: str) -> bytes:
    return subprocess.run(
        ["git", "-C", str(repo), "-c", "gc.auto=0", *arguments], capture_output=True, check=True
    ).stdout
...
def _build_repo(repo: Path) -> None:
    ...
    subprocess.run(
        ["git", "-C", str(repo), "commit", "-q", "-m", "fixture"], env=environment, check=True
    )
...
def _materialize_cue_set_subprocess(
    repo: Path,
    universe: Path,
    d1_sha: str,
    ...
    payload = {
        ...
        "output": str(output),
        "input_current": 18.0,
        "model": FIXTURE_MODEL.to_dict(),
        "encoder": {
            "feature_dim": FIXTURE_ENCODER.feature_dim,
            "packet_ms": FIXTURE_ENCODER.packet_ms,
            "silent_ms": FIXTURE_ENCODER.silent_ms,
            "active_fraction": FIXTURE_ENCODER.active_fraction,
            "projection_seed": FIXTURE_ENCODER.projection_seed,
        },
    }
    process = subprocess.run(
        [sys.executable, "-c", _D2_SUBPROCESS, json.dumps(payload)],
        capture_output=True,
        env=environment,
        check=False,
    )
...
def _acquire_d1_d2(
    workspace: Path, encoder_checkpoint: Path, encoder_digest: str, environment: dict[str, str]
) -> dict[str, Any]:
    ...
    ids = [
        str(record_id) for record_id in cue_set.payload["source_universe"]["selected_record_ids"]
    ]
    assert len(ids) >= 3, (
        f"the fixture cue set must declare at least three candidates, got {len(ids)}"
    )
    return {
        "repo": repo,
        "universe_path": universe_path,
        "d1_file_sha256": d1_result.file_sha256,
        "cue_set_path": output / "cue_set.json",
        "d2_file_sha256": d2_file_sha256,
        "ids": ids,
    }
...
def _base_config(arguments: argparse.Namespace, fixture: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "checkpoint_schema_sha256": _sha(
            (schema_dir / "snn_memory_checkpoint_v2.schema.json").read_bytes()
        ),
        "bank_schema_sha256": _sha(
            (schema_dir / "snn_memory_candidate_bank_v2.schema.json").read_bytes()
        ),
...
def _calibrate_argv(
    ...
    *,
    ...
    return [
        ...
        "--weights-digest",
        digest(worker_dir / "weights.bin"),
        "--topology",
        str(worker_dir / "topology.bin"),
        "--topology-digest",
        digest(worker_dir / "topology.bin"),
        "--cue",
        str(cue),
        "--cue-digest",
        cue_digest if cue_digest is not None else digest(Path(cue)),
        "--model-config",
        str(worker_dir / "model_config.json"),
        "--model-config-digest",
        digest(worker_dir / "model_config.json"),
        "--record-id",
        record_id,
        "--nonce",
        os.urandom(16).hex(),
        ...
        backend_build_digest if backend_build_digest is not None else extension_sha256,
    ]
...
def _adversarial_signature_witness() -> None:
    ...
    bin_major = [
        float(raster[b * per_bin : (b + 1) * per_bin, neuron].sum())
        for b in range(8)
    ...
    _expect(
        "at least 32 steps and divisible by eight",
        lambda: temporal_signature_v2(np.zeros((24, n), dtype=np.bool_), 24, n),
    )
    _expect(
        "completion raster shape mismatch",
        lambda: temporal_signature_v2(
            np.zeros((COMPLETION_STEPS, n + 1), dtype=np.bool_), COMPLETION_STEPS, n
        ),
    )
...
def _check_resealed_forgeries(checkpoint_dir: Path, scratch: Path, untrained_dir: Path) -> None:
    ...
    _expect(
        "sealed topology differs from the seed-derived connectivity",
        lambda: cbv.read_checkpoint_bundle_v2(
            _reseal_checkpoint(
                checkpoint_dir,
                scratch / "rf_topology",
                "topology",
                forged_topology.tobytes(),
                cbv.topology_digest(forged_topology, n_exc),
            )
        ),
    )
    ...
    _expect(
        "an excitatory row carries a negative weight",
        lambda: cbv.read_checkpoint_bundle_v2(
            _reseal_checkpoint(
                checkpoint_dir,
                scratch / "rf_dale",
                "weights",
                weights.tobytes(),
                cbv.checkpoint_component_digest("weights", "<f8", weights),
            )
        ),
    )
    ...
    _expect(
        ...
        lambda: cbv.read_checkpoint_bundle_v2(
            _reseal_checkpoint(
                ...
                scratch / "rf_csr",
                "adjacency_outgoing",
                offsets.tobytes() + indices.tobytes(),
                cbv._csr_digest("outgoing", csr),
            )
        ),
    )
    ...
    schedule[0]["record_id"], schedule[1]["record_id"] = (
        schedule[1]["record_id"],
        schedule[0]["record_id"],
    )
    _expect(
        "replay order breaks the seeded permutation",
        lambda: cbv.read_checkpoint_bundle_v2(
            _reseal_checkpoint(
                checkpoint_dir,
                scratch / "rf_replay",
                "replay_schedule",
                cbv._canonical_bytes(schedule),
                cbv.replay_schedule_digest(schedule),
            )
        ),
    )
    ...
    _expect(
        "signatures carry a non-finite value",
        lambda: cbv.read_checkpoint_bundle_v2(
            _reseal_checkpoint(
                checkpoint_dir,
                scratch / "rf_sig",
                "signatures",
                signatures.tobytes(),
                cbv.checkpoint_component_digest("signatures", "<f8", signatures),
            )
        ),
    )
    ...
    _expect(
        "probe-initial state voltage is not the fresh resting potential",
        lambda: cbv.read_checkpoint_bundle_v2(
            _reseal_checkpoint(
                checkpoint_dir,
                scratch / "rf_probe",
                "probe_initial_state",
                body("training_final_state"),
                semantic("training_final_state"),
            )
        ),
    )
    ...
    _expect(
        "exceeds the sealed weight bound",
        lambda: cbv.read_checkpoint_bundle_v2(
            _reseal_checkpoint(
                checkpoint_dir,
                scratch / "rf_bound",
                "weights",
                over.tobytes(),
                cbv.checkpoint_component_digest("weights", "<f8", over),
            )
        ),
    )
    ...
    in_csr = cbv.CsrArrays(
        offsets=np.ascontiguousarray(in_off, dtype="<u8"),
        indices=np.ascontiguousarray(in_idx, dtype="<u8"),
    )
    _expect(
        "incoming adjacency row",
        lambda: cbv.read_checkpoint_bundle_v2(
            _reseal_checkpoint(
                checkpoint_dir,
                scratch / "rf_incsr",
                "adjacency_incoming",
                in_off.tobytes() + in_idx.tobytes(),
                cbv._csr_digest("incoming", in_csr),
            )
        ),
    )
    ...
    _expect(
        "topology component declares an unexpected dtype",
        lambda: cbv.read_checkpoint_bundle_v2(
            _reseal_descriptor_field(
    ...
    _expect(
        "topology component declares an unexpected framing version",
        lambda: cbv.read_checkpoint_bundle_v2(
            _reseal_descriptor_field(
    ...
    _expect(
        "boolean bytes are not exactly zero or one",
        lambda: cbv.read_checkpoint_bundle_v2(
            _reseal_checkpoint(
                checkpoint_dir,
                scratch / "rf_rawbool",
                "topology",
                raw_bool.tobytes(),
                cbv.topology_digest(boolean, n_exc),
            )
        ),
    )
    ...
    _expect(
        "descriptor seed disagrees with the manifest",
        lambda: cbv.read_checkpoint_bundle_v2(
            _reseal_manifest_field(checkpoint_dir, scratch / "rf_seed", swap_seed)
        ),
    )
    ...
    _expect(
        "topology carries a self-connection on the diagonal",
        lambda: cbv.read_checkpoint_bundle_v2(
            _reseal_checkpoint(
                checkpoint_dir,
                scratch / "rf_diag_topo",
                "topology",
                diagonal_topology.tobytes(),
                cbv.topology_digest(diagonal_topology, n_exc),
            )
        ),
    )
    ...
    _expect(
        "weights carry a self-connection on the diagonal",
        lambda: cbv.read_checkpoint_bundle_v2(
            _reseal_checkpoint(
                checkpoint_dir,
                scratch / "rf_diag_w",
                "weights",
                diag_weights.tobytes(),
                cbv.checkpoint_component_digest("weights", "<f8", diag_weights),
            )
        ),
    )
    ...
    _expect(
        "weights carry a non-finite value",
        lambda: cbv.read_checkpoint_bundle_v2(
            _reseal_checkpoint(
                checkpoint_dir,
                scratch / "rf_inf_w",
                "weights",
                inf_weights.tobytes(),
                cbv.checkpoint_component_digest("weights", "<f8", inf_weights),
            )
        ),
    )
    ...
    _expect(
        "weights component shape mismatch",
        lambda: cbv.read_checkpoint_bundle_v2(
            _reseal_descriptor_field(
        ...
        _expect(
            ...
            lambda: cbv.read_checkpoint_bundle_v2(
                _reseal_checkpoint(
                    ...
                    scratch / tag,
                    "replay_schedule",
                    cbv._canonical_bytes(forged),
                    cbv.replay_schedule_digest(forged),
                )
            ),
        )
    ...
    _forge_replay("rf_replay_perm", _dup_position, "replay positions are not a full permutation")

    # 19. Replay entry carries a non-positive timestep count.
    _forge_replay(
        "rf_replay_ts",
        lambda s: s[0].__setitem__("timesteps", 0),
    ...
    _expect(
        "topology component shape mismatch",
        lambda: cbv.read_checkpoint_bundle_v2(
            _reseal_descriptor_field(
    ...
    _expect(
        "adjacency_outgoing component shape mismatch",
        lambda: cbv.read_checkpoint_bundle_v2(
            _reseal_descriptor_field(
    ...
    _expect(
        "signatures component shape mismatch",
        lambda: cbv.read_checkpoint_bundle_v2(
            _reseal_descriptor_field(
                checkpoint_dir,
                scratch / "rf_dc_sig",
    ...
    def _forge_identity(tag: str, mutate: Callable[[dict[str, Any]], None], error: str) -> None:
        _expect(
            error,
            lambda: cbv.read_checkpoint_bundle_v2(
                _reseal_descriptor_identities(checkpoint_dir, scratch / tag, mutate)
            ),
        )

    _forge_identity(
        "rf_id_nn",
        lambda idn: idn.__setitem__("n_neurons", int(idn["n_neurons"]) + 1),
        "descriptor neuron count disagrees with the model configuration",
    )
    _forge_identity(
        "rf_id_ne",
        lambda idn: idn.__setitem__("n_excitatory", int(idn["n_excitatory"]) + 1),
        "descriptor excitatory count disagrees with the model configuration",
    )
    _forge_identity(
        "rf_id_wm",
        lambda idn: idn.__setitem__("weight_max", float(idn["weight_max"]) + 1.0),
    ...
    _expect(
        "component weights path mismatch",
        lambda: cbv.read_checkpoint_bundle_v2(
            _reseal_descriptor_field(
    ...
    _expect(
        "component weights semantic digest mismatch",
        lambda: cbv.read_checkpoint_bundle_v2(
            _reseal_descriptor_field(
    ...
    _expect(
        "descriptor replay-schedule digest mismatch",
        lambda: cbv.read_checkpoint_bundle_v2(
            _reseal_descriptor(checkpoint_dir, scratch / "rf_replaydig", _wrong_replay_digest)
        ),
    )
    ...
    def _forge_state(role: str, tag: str, offset: int, patch: bytes, error: str) -> None:
        ...
        _expect(
            error,
            lambda: cbv.read_checkpoint_bundle_v2(
                _reseal_checkpoint(checkpoint_dir, scratch / tag, role, forged, semantic)
            ),
        )
    ...
    _forge_state(
        ...
        "state voltage is not finite <f8",
    )
    _forge_state(
        "probe_initial_state",
        "rf_probe_refrac",
        n * 8,
        np.uint32(1).tobytes(),
        "refractory or spike field is not a fresh reset",
    )
    _forge_state(
        "probe_initial_state",
        "rf_probe_trace",
        ...
        "synaptic trace field is not a fresh reset",
    )
    ...
    def _forge_replay_bytes(tag: str, raw: bytes, error: str) -> None:
        _expect(
            error,
            lambda: cbv.read_checkpoint_bundle_v2(
                _reseal_checkpoint(checkpoint_dir, scratch / tag, "replay_schedule", raw, "0" * 64)
            ),
        )
    ...
    _forge_replay_bytes(
        "rf_rj_keys", b'[{"epoch":0}]\n', "replay entry does not carry the exact schema keys"
    )
    _forge_replay_bytes(
        "rf_rj_int",
        b'[{"epoch":"0","record_id":"r","replay_position":0,"timesteps":1}]\n',
        "must be a JSON integer",
    )
    _forge_replay_bytes(
        "rf_rj_str",
        b'[{"epoch":0,"record_id":1,"replay_position":0,"timesteps":1}]\n',
    ...
    _expect(
        "an inhibitory row carries a positive weight",
        lambda: cbv.read_checkpoint_bundle_v2(
            _reseal_checkpoint(
                checkpoint_dir,
                scratch / "rf_w_inhpos",
                "weights",
                inh_weights.tobytes(),
                cbv.checkpoint_component_digest("weights", "<f8", inh_weights),
            )
        ),
    )
    ...
    _expect(
        "path carries a non-normal '..' component",
    ...
    _expect(
        "checkpoint bundle target has no valid final component",
        lambda: _write(scratch / "bad" / ".."),
    )

    # 55-56. Writer witnesses for CSR bracket and duplicate ordered record identifiers.
    _expect(
        "adjacency offsets do not bracket the indices",
        lambda: _write_inputs(
            "wt_csr_bracket",
            outgoing=cbv.CsrArrays(
    ...
    _expect(
        "ordered record identifiers are not unique",
    ...
    _expect(
        "component is shorter than the offset row",
        lambda: cbv.read_checkpoint_bundle_v2(
            _reseal_adjacency_shape(checkpoint_dir, scratch / "rf_adj_short", n_short.tobytes())
        ),
    )
    ...
    _expect(
        "checkpoint manifest is not canonical",
        lambda: cbv.read_checkpoint_bundle_v2(
            _reseal_noncanonical_manifest(checkpoint_dir, scratch / "rf_noncanon_manifest")
        ),
    )
    ...
    _expect(
        "an untrained checkpoint must carry no replay entries",
        lambda: cbv.read_checkpoint_bundle_v2(
            _reseal_checkpoint(
                untrained_dir,
                scratch / "rf_untr_replay",
                "replay_schedule",
    ...
    _expect(
        "checkpoint manifest payload digest mismatch",
        lambda: cbv.read_checkpoint_bundle_v2(
            _reseal_descriptor(
                checkpoint_dir,
                scratch / "rf_payload",
    ...
    _expect(
        "component inventory is out of order",
        lambda: cbv.read_checkpoint_bundle_v2(
            _reseal_descriptor(checkpoint_dir, scratch / "rf_order", _reorder)
        ),
    )
    ...
    _expect(
        "state component byte length mismatch",
        lambda: cbv.read_checkpoint_bundle_v2(
            _reseal_checkpoint(
                checkpoint_dir,
                scratch / "rf_state_len",
                "training_final_state",
                short_state,
                "0" * 64,
            )
    ...
    _expect(
        "adjacency offsets length mismatch",
        lambda: _write_inputs(
            "wt_csr_len",
            outgoing=cbv.CsrArrays(
    ...
    _expect(
        "float fields have the wrong length",
        lambda: _write_inputs(
            "wt_state_flen",
            training_final_state=cbv.StateArrays(
                voltage_mv=np.zeros(n - 1, dtype=np.float64),
                refractory_steps=ostate.refractory_steps,
                spikes=ostate.spikes,
                pre_trace=ostate.pre_trace,
                post_trace=ostate.post_trace,
            ),
        ),
    )
    _expect(
        "spike field has the wrong length",
        lambda: _write_inputs(
            "wt_state_slen",
            training_final_state=cbv.StateArrays(
                voltage_mv=ostate.voltage_mv,
                refractory_steps=ostate.refractory_steps,
                spikes=np.zeros(n - 1, dtype=np.bool_),
                pre_trace=ostate.pre_trace,
                post_trace=ostate.post_trace,
            ),
        ),
    ...
    _expect(
        "refractory or trace fields have the wrong length",
        lambda: _write_inputs(
            "wt_state_rlen",
            training_final_state=cbv.StateArrays(
                voltage_mv=ostate.voltage_mv,
                refractory_steps=np.zeros(n - 1, dtype=np.uint32),
                spikes=ostate.spikes,
                pre_trace=ostate.pre_trace,
                post_trace=ostate.post_trace,
            ),
    ...
    _expect(
        "refractory or spike dtype mismatch",
        lambda: _write_inputs(
            "wt_state_dtype",
            training_final_state=cbv.StateArrays(
                voltage_mv=ostate.voltage_mv,
                refractory_steps=np.zeros(n, dtype=np.int64),
                spikes=ostate.spikes,
                pre_trace=ostate.pre_trace,
                post_trace=ostate.post_trace,
            ),
    ...
    _expect(
        ...
        lambda: cbv.read_checkpoint_bundle_v2(
            _reseal_narrow_signatures(checkpoint_dir, scratch / "rf_sig_width", narrow)
        ),
    )
    ...
    ooff[1] = int(ooff[2]) + 1
    ocsr = cbv.CsrArrays(
        offsets=np.ascontiguousarray(ooff, dtype="<u8"),
    ...
    _expect(
        ...
        lambda: cbv.read_checkpoint_bundle_v2(
            _reseal_checkpoint(
                ...
                scratch / "rf_csr_mono",
                "adjacency_outgoing",
                ooff.tobytes() + oidx.tobytes(),
                cbv._csr_digest("outgoing", ocsr),
            )
        ),
    )
...
def _reseal_descriptor_identities(
    src: Path, dst: Path, mutate: Callable[[dict[str, Any]], None]
) -> Path:
...
def _check_resealed_bank_forgeries(bank_dir: Path, scratch: Path) -> None:
    def calibration_swap(_dst: Path, manifest: dict[str, Any]) -> None:
        ...
        calibration[0]["record_id"], calibration[1]["record_id"] = (
            calibration[1]["record_id"],
            calibration[0]["record_id"],
        )

    _expect(
        "bank calibration order or count differs from the ordered records",
    ...
    _expect(
        "signature layout and scoring completion windows disagree",
        lambda: cbank.read_candidate_bank_v2(
    ...
    _expect(
        "bank build bank_schema_sha256 differs from the packaged schema hash",
        lambda: cbank.read_candidate_bank_v2(
    ...
    _expect(
        "bank calibration entries disagree on encoder_identity",
        lambda: cbank.read_candidate_bank_v2(
    ...
    _expect(
        "bank calibration encoder config differs from the build identity",
        lambda: cbank.read_candidate_bank_v2(
    ...
    _expect(
        "bank calibration current shape is not (timesteps, n_neurons)",
        lambda: cbank.read_candidate_bank_v2(
    ...
    _expect(
        "bank calibration weights changed under disabled plasticity",
        lambda: cbank.read_candidate_bank_v2(
    ...
    def signature_value_drift(dst: Path, manifest: dict[str, Any]) -> None:
        ...
        array[0] = (
            array[0] + 1.0
        )  # finite change: passes canonical, fails the stored raw-byte digest
        forged = np.ascontiguousarray(array, dtype="<f8").tobytes()
        (dst / "signatures.bin").write_bytes(forged)
        manifest["signatures"]["file_sha256"] = _sha(forged)
    ...
    _expect(
        "signature raw-byte digest mismatch",
        lambda: cbank.read_candidate_bank_v2(
    ...
    _expect(
        "signature byte length differs from the declared bank shape",
        lambda: cbank.read_candidate_bank_v2(
    ...
    good_signatures = (
        np.frombuffer((bank_dir / "signatures.bin").read_bytes(), dtype="<f8")
        .reshape(rows, width)
        .copy()
    )

    def _write(target_name: str, manifest: dict[str, Any], signatures: Any) -> object:
        return cbank.write_candidate_bank_v2(
            scratch / target_name, manifest=manifest, signatures=signatures
        )

    _expect(
        "written signatures shape differs from the ordered record bank",
        lambda: _write(
            "w_shape", json.loads(json.dumps(base_manifest)), good_signatures.reshape(rows * width)
        ),
    )
    _expect(
        "is not a canonical <f8 array",
        lambda: _write(
            "w_dtype", json.loads(json.dumps(base_manifest)), good_signatures.astype("<f4")
        ),
    )
    _expect(
        "is not C-contiguous",
    ...
    _expect(
        "signature row record identifier is not a string",
    ...
    _expect(
        "candidate bank target has no valid final component",
...
def _witness_bank_rename_eacces(scratch: Path, manifest: dict[str, Any], signatures: Any) -> None:
    ...
    try:
        cbank.write_candidate_bank_v2(
            target, manifest=json.loads(json.dumps(manifest)), signatures=signatures.copy()
        )
    ...
    if not (
        ...
        and "atomic no-replace install failed" in message
    ...
    if target.exists():
        raise AssertionError(
            "bank rename EACCES witness accepted an output despite the failed rename"
        )
    if not residue:
        raise AssertionError(
            "bank rename EACCES witness expected staging residue under the unwritable parent"
        )
    shutil.rmtree(parent, ignore_errors=True)
...
def _check_checkpoint_negatives(bundle_dir: Path, scratch: Path) -> None:
    def break_component(directory: Path) -> None:
        (directory / "weights.bin").write_bytes(
            b"\x00" * (directory / "weights.bin").stat().st_size
        )

    _expect(
        "weights file digest or length mismatch",
        lambda: cbv.read_checkpoint_bundle_v2(
            _tamper(bundle_dir, scratch / "c_weights", break_component)
        ),
    )
    ...
    _expect(
        "checkpoint manifest file digest mismatch",
        lambda: cbv.read_checkpoint_bundle_v2(
            _tamper(bundle_dir, scratch / "c_manifest", break_manifest)
        ),
    )
    ...
    _expect(
        "directory inventory does not match the descriptor",
        lambda: cbv.read_checkpoint_bundle_v2(
            _tamper(bundle_dir, scratch / "c_drop", drop_component)
        ),
    )
    ...
    _expect(
        "directory inventory does not match the descriptor",
        lambda: cbv.read_checkpoint_bundle_v2(
            _tamper(bundle_dir, scratch / "c_extra", add_component)
        ),
    )
    ...
    _expect(
        "cannot be opened safely",
        lambda: cbv.read_checkpoint_bundle_v2(
            _tamper(bundle_dir, scratch / "c_link", symlink_component)
        ),
    )
    ...
    _expect(
        "descriptor self digest mismatch",
        lambda: cbv.read_checkpoint_bundle_v2(
            _tamper(bundle_dir, scratch / "c_desc", break_descriptor)
        ),
    )
...
def _check_bundle_reader_negatives(bundle_dir: Path, scratch: Path) -> None:
    for index, (content, error) in enumerate(_JSON_VARIANTS):
        ...
        _expect(
            error,
            lambda mutation=mutate, tag=index: cbv.read_checkpoint_bundle_v2(
                _tamper(bundle_dir, scratch / f"dr_{tag}", mutation)
            ),
        )
    ...
    _expect(
        "is not a regular file",
        lambda: cbv.read_checkpoint_bundle_v2(
            _tamper(bundle_dir, scratch / "dr_dir", descriptor_dir)
        ),
    )
...
def _check_bank_negatives(bank_dir: Path, scratch: Path) -> None:
    ...
    for index, (content, error) in enumerate(_JSON_VARIANTS):
        ...
        _expect(
            error,
            lambda mutation=mutate, tag=index: cbank.read_candidate_bank_v2(
                _tamper(bank_dir, scratch / f"br_{tag}", mutation)
            ),
        )
    ...
    _expect(
        "candidate bank manifest is not canonical",
    ...
    _expect(
        "is not a regular file",
        lambda: cbank.read_candidate_bank_v2(
            _tamper(bank_dir, scratch / "br_sigdir", signature_dir)
        ),
    )
    ...
    _expect(
        "candidate bank target already exists",
    ...
    _expect(
        "does not recompute from the ordered rows",
        lambda: cbank.write_candidate_bank_v2(
            scratch / "bw_bad", manifest=bad, signatures=signatures
        ),
    )
    (scratch).mkdir(parents=True, exist_ok=True)
    (scratch / "not_a_dir").write_bytes(b"x")
    _expect(
        "candidate bank directory cannot be opened safely",
...
def _check_signature_negatives(bank_dir: Path, scratch: Path) -> None:
    ...
    _expect(
        "signature file digest or length mismatch",
        lambda: cbank.read_candidate_bank_v2(
            _tamper(bank_dir, scratch / "b_sig", break_signatures)
        ),
    )
    ...
    _expect(
        "candidate bank self digest mismatch",
    ...
    _expect(
        "inventory is not exactly bank.json and signatures.bin",
        lambda: cbank.read_candidate_bank_v2(_tamper(bank_dir, scratch / "b_extra", add_file)),
    )


def _check_materialize_negatives(
    base: dict[str, Any],
    workspace: Path,
    trained_bundle: Path,
    untrained_bundle: Path,
    worker_dir: Path,
    extension_sha256: str,
    crate_version: str,
    environment: dict[str, str],
    ...
    for forbidden in (
        ...
        run_neg(
            lambda c, value=forbidden: c.__setitem__("condition", value),
            f"forbidden_{forbidden}",
            "is forbidden in D4-A",
        )
    run_neg(
        lambda c: c.__setitem__("condition", "bogus"),
        "unknown",
        "is not an accepted D4-A condition",
    )
    for field in (
        "evaluation_cues",
    ...
    run_neg_raw(
        lambda r: r.replace(b'"condition":', b'"_nan":NaN,"condition":', 1),
        "nan",
        "config contains a non-finite JSON constant",
    )
    run_neg_raw(
        lambda r: json.dumps(json.loads(r), indent=2).encode("utf-8"),
        "noncanon",
    ...
    run_neg(
        lambda c: c.__setitem__("repository_head", "0" * 40),
        "rephead",
        "D1 repository HEAD differs from the declared repository head",
    )
    run_neg(
        lambda c: c.__setitem__("encoder_config_digest", "0" * 64),
        "enccfgdig",
        "declared encoder-config digest differs from the D2 encoder configuration",
    )
    run_neg(
        lambda c: c["encoder_config"].__setitem__(
            "max_events", int(c["encoder_config"].get("max_events", 8)) + 1
        ),
        "enccfg",
        "declared encoder config differs from its declared digest",
    )
    run_neg(
        lambda c: c.__setitem__("input_current", float(c["input_current"]) + 1.0),
        "incur",
        "declared input current differs from the D2 model input current",
    )
    run_neg(
        lambda c: c.__setitem__("d1", "/nonexistent_d4a_probe/d1.json"),
        "badd1parent",
        "cannot be opened safely",
    )
    run_neg(
        lambda c: c.__setitem__("backend_build_digest", "0" * 64),
        "bbd",
        "declared backend-build digest is not the loaded extension identity",
    )
    run_neg(
        lambda c: c.__setitem__("output", "/"),
        "outroot",
        "materialization output has no valid final component",
    )
    run_neg(
        lambda c: (
            c.__setitem__("condition", "shuffled"),
            c.__setitem__("seed", 29),
            c.__setitem__("trained_bundle", str(trained_bundle)),
        ),
        "controlseed",
    ...
    run_neg(
        lambda c: (
            c.__setitem__("condition", "shuffled"),
            c.__setitem__("trained_bundle", str(untrained_bundle)),
        ),
        "notrained",
        "a control must derive from a trained checkpoint",
    )
    run_neg(
        lambda c: c.__setitem__("output", str(untrained_bundle.parent)),
        "clobber",
    ...
    _run_cli_fail(
        ["materialize", "--config", str(workspace / "d4a-repo/../neg_forbidden.json")],
        environment,
    ...
    _run_cli_fail(
        _calibrate_argv(worker_dir, bad_cue, "sha256:" + "0" * 64, extension_sha256, crate_version),
        environment,
        "not a whole (timesteps, n_neurons) grid",
    )
    _run_cli_fail(
        _calibrate_argv(
            worker_dir,
            worker_dir / "cue_000.bin",
            "sha256:" + "0" * 64,
            extension_sha256,
            crate_version,
            cue_digest="0" * 64,
    ...
    _run_cli_fail(
        _calibrate_argv(
            ...
            extension_sha256,
            crate_version,
            backend_build_digest="0" * 64,
        ),
        environment,
        "worker backend-build digest is not the loaded extension identity",
    )
    ...
    def calib_neg(
        ...
        *,
        ...
        cue: bytes | None = None,
        completion: int = COMPLETION_STEPS,
    ) -> None:
    ...
    calib_neg(
        "mc_nan",
        "worker model-config carries a non-finite JSON constant",
        model=b'{"_nan":NaN,' + _canon_model(real_model)[1:],
    )
    calib_neg(
        "mc_dupkey",
        "worker model-config contains a duplicate key",
    ...
    calib_neg(
        "mc_noncanon",
        "worker model-config is not canonical JSON",
    ...
    calib_neg(
        "mc_partial",
        "worker model config is not the full canonical configuration",
    ...
    cue_nan[0] = np.nan
    calib_neg(
        "cue_nan", "calibration cue current carries a non-finite value", cue=cue_nan.tobytes()
    )
    ...
    assert _selected_records and all(
        int(record["event_count"]) >= _su.MIN_EVENTS for record in _selected_records
    ), (
    ...
    def prep_neg(
        ...
        *,
        ...
        cfg_over: Callable[[dict[str, Any]], None] | None = None,
    ) -> None:
    ...
    prep_neg(
        "xbind",
        "cue set does not cross-bind the exact D1 artifact and HEAD",
        mut_d2=lambda p, dd: p["source_universe"].__setitem__("file_sha256", "0" * 64),
    )
    prep_neg(
        "encdir",
        "D2 encoder directory digest differs from the pinned encoder digest",
        mut_d2=lambda p, dd: p["encoder"].__setitem__("directory_sha256", "0" * 64),
    )
    prep_neg(
        "normdig",
        "calibration cue normalized digest for",
    ...
    prep_neg(
        "livehead",
        "live repository HEAD differs from the authenticated D1 HEAD",
        mut_d1=mut_livehead,
    ...
    def realign_d2_records(p2: dict[str, Any], _forged_d2: Path) -> None:
        ...
        p2["records"] = [
            by_id[rid] for rid in p2["source_universe"]["selected_record_ids"] if rid in by_id
        ]

    prep_neg(
        "gitpath", "nonexistent-d4a-record.md failed", mut_d1=mut_gitpath, mut_d2=realign_d2_records
    )
    ...
    prep_neg("cuepath", "traverses unsafely", mut_d2=lambda p, dd: shutil.rmtree(dd / "cues"))
    ...
    _orig_d2 = next(
        r for r in json.loads(d2_src.read_bytes())["records"] if r["record_id"] == _orig_rid0
    )
    ...
    for _n in range(500000):
        ...
        _csha = _sha(_cand)
        if (
            _cue._calibration_block_index(_csha) == _tgt_block
            and _cue._family_permutation_index(_csha) == _tgt_perm
        ):
            _bin_bytes = _cand
            break
    assert _bin_bytes and len(_bin_bytes) <= 20000, (
        "no block+permutation-matching non-UTF8 content found"
    )
    _bin_path = "docs/public/record-99.md"
    (_bin_repo / _bin_path).write_bytes(_bin_bytes)
    _git(_bin_repo, "add", "-A")
    _commit_env = dict(
        os.environ, GIT_AUTHOR_DATE="@1700000000 +0000", GIT_COMMITTER_DATE="@1700000000 +0000"
    )
    subprocess.run(
        ["git", "-C", str(_bin_repo), "commit", "-q", "--amend", "--no-edit"],
        env=_commit_env,
        check=True,
    )
    ...
    def mut_binary_d2(p2: dict[str, Any], _forged_d2: Path) -> None:
        ...
        if (
            _orig_rid0 in by_id
        ):  # block+perm already match by search -> repurpose VERBATIM, only record_id
            by_id[_bin_rid] = dict(by_id[_orig_rid0], record_id=_bin_rid)
        p2["records"] = [
            by_id[rid] for rid in p2["source_universe"]["selected_record_ids"] if rid in by_id
        ]

    def cfg_binary(c: dict[str, Any]) -> None:
        c["repo"] = str(_bin_repo)
        c["repository_head"] = _bin_head

    prep_neg(
        "binaryblob",
        "is not strict UTF-8",
        mut_d1=mut_binary,
        mut_d2=mut_binary_d2,
        cfg_over=cfg_binary,
    )
    ...
    try:
        run_neg(
            lambda c: c.__setitem__("output", str(final_output)),
            "renameat2race",
            "materialization output already exists",
        )
    finally:
    ...
    try:
        run_neg(
            lambda c: c.__setitem__("output", str(eacces_final)),
            "renameat2eacces",
    ...
    assert residue, (
        "EACCES witness expected an inert staging residue in the caller-supplied output parent"
    )
    ...
    run_neg(
        lambda c: c.__setitem__("d1", str(nonreg_dir)),
        "nonreg283",
    ...
    def control_neg(
        ...
        *,
        ...
        mut_desc_ids: Callable[[dict[str, Any]], None] | None = None,
    ) -> None:
    ...
    control_neg(
        "identity",
        "trained bundle experiment_digest differs from the control configuration",
        mut_manifest=set_experiment,
        mut_desc_ids=set_experiment,
    )
    ...
    run_neg(
        lambda c: c.__setitem__("output", str(linked_parent / "sym_out")),
        "symparent",
        "cannot be opened safely",
    )
...
def _check_bind_negatives(
    workspace: Path, results: dict[int, dict[str, Any]], environment: dict[str, str]
) -> None:
    seed_a, seed_b = SEEDS
    ...
    bind_neg(
        "exp",
        lambda _d, m: m["identities"].__setitem__("experiment_digest", "0" * 64),
        "candidate bank experiment digest differs from the checkpoint",
    )
    bind_neg(
        "calcue",
        lambda _d, m: m["calibration"][0].__setitem__("source_cue_sha256", "0" * 64),
        "calibration source cue digest for",
    )
    bind_neg(
        "calmodel", all_calibration("model_config_digest", "0" * 64), "calibration model config for"
    )
    bind_neg(
        "calcurrent", all_calibration("input_current", 0.123456789), "calibration input current for"
    )
    ...
    scoring_neg(
        "order",
        "scoring-target candidate order differs from the lexical bank order",
        reorder_candidates,
    )
    scoring_neg(
        "signatures", "scoring-target signature digests differ from the bank rows", mutate_signature
    )
    scoring_neg(
        "completion",
        "scoring-target completion window differs from the bank layout",
...
def _prove_installed_origins(arguments: argparse.Namespace) -> None:
    ...
    for name in ("checkpoint_bundle_v2", "candidate_bank_v2", "checkpoint_materialize_v2"):
        ...
        assert _sha(Path(origin).read_bytes()) == _sha(
            (root / "snn_memory" / f"{name}.py").read_bytes()
    ...
    for base in (
        ...
        assert packaged == (root / "docs/schema" / base).read_bytes(), (
            f"{base} packaged != docs/schema"
        )
        assert files("snn_memory").joinpath("schema", base + ".license").read_bytes(), (
            f"{base} licence missing"
        )
    import rust_snn_memory.rust_snn_memory as extension

    ext_origin = str(Path(extension.__file__).resolve())
    assert ext_origin.startswith(install + "/"), (
        f"extension origin {ext_origin} not under install prefix"
    )
    assert _sha(Path(ext_origin).read_bytes()) == arguments.extension_sha256, (
        "extension digest drift"
    )
...
def main() -> int:
    ...
    fixture = _acquire_d1_d2(
        workspace, arguments.encoder_checkpoint, arguments.encoder_digest, environment
    )
    ...
    descriptors = [
        results[s][c]["checkpoint_descriptor_self_sha256"] for s in SEEDS for c in CONDITIONS
    ]
    banks = [results[s][c]["bank_self_sha256"] for s in SEEDS for c in CONDITIONS]
    assert len(set(descriptors)) == len(descriptors), (
        "checkpoint descriptors are not per-(seed,condition) unique"
    )
    ...
    for seed in SEEDS:
        for condition in ("shuffled", "random", "zero"):
            assert (
                results[seed][condition]["epochs_completed"]
                == results[seed]["trained"]["epochs_completed"]
            )
    ...
    calibrate_report = _run_cli(
        _calibrate_argv(
            worker_dir,
            worker_dir / "cue_000.bin",
            results[SEEDS[0]]["trained"]["ordered_record_ids"][0],
            arguments.extension_sha256,
            str(base["crate_version"]),
        ),
        environment,
    )
    ...
    _run_cli_fail(
        ["materialize", "--config", str(workspace / "does-not-exist.json")],
        environment,
        "cannot be opened safely",
    )
    ...
    assert (
        replay["checkpoint_descriptor_self_sha256"]
        == results[SEEDS[0]]["trained"]["checkpoint_descriptor_self_sha256"]
    ), "byte-determinism replay produced a different checkpoint"
        ...
        json.dumps(
            {
                ...
                "checkpoint_descriptors": {
                    f"{s}_{c}": results[s][c]["checkpoint_descriptor_self_sha256"]
                    for s in SEEDS
                    for c in CONDITIONS
                },
            },
