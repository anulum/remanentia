...
def _build_repo(repo: Path, salt: bytes) -> None:
    ...
    _git(repo, "add", "-A")
    env = dict(os.environ, GIT_AUTHOR_DATE=f"@{BASE_TIMESTAMP} +0000",
...
def _acquire_d1_d2(workspace: Path, repo: Path, encoder_checkpoint: Path,
                   encoder_digest: str) -> tuple[Any, Any, str, list[Any]]:
    ...
    d2_result = materialize_cue_set(
        repo, universe_path, d1_result.file_sha256, encoder_checkpoint, encoder_digest, output,
...
def _check_bundle_completeness_negatives(lock: Any, scoring: Any, calibration: Any, d1_art: Any,
                                         d2_art: Any, bundle_arts: list[Any],
                                         foreign_bundle: Any) -> None:
...
def _d2_binding(d1_binding: dict[str, Any], d2_art: Any, bundle_inventory_digest: str) -> dict[str, Any]:
    return {
...
def _scoring_target(record_ids: list[str], bindings: dict[str, str]) -> dict[str, Any]:
    ...
    base = {
        ...
        "state": "fixture_only", "lane_role": "lane_p", "completion_steps": 32, "bins": 8,
...
def _calibration_spec(bindings: dict[str, str]) -> dict[str, Any]:
    ...
    return {
        "schema_version": 2, "artifact_type": "snn-memory-gb-calibration-spec-v2",
        "state": "fixture_only", "lane_role": "lane_p",
        "epsilon": 2.220446049250313e-16, "epsilon_bits": "3cb0000000000000",
        "numerical_zero_floor_grid": [1e-09], "completion_steps": 32, "bins": 8,
        "tail_bins": [5, 6, 7, 8], "candidate_lags": [1, 2, 3, 4],
        "spike_drift_ceiling_grid": [0.05], "current_drift_ceiling_grid": [0.05],
        "representation_margin_floor_grid": [0.05],
        "normalized_effective_rank_floor_grid": [0.1], "settled_fraction_floor_grid": [0.5],
        "wandering_ceiling_grid": [0.3], "collapse_ceiling_grid": [0.3], "abstention_grid": [0.0],
        "selection_objective": "macro-youden-j", "tie_rule": "ascending-canonical-json",
        "validation_floors": {"sensitivity_min": 0.8, "specificity_min": 0.95, "false_recall_max": 0.05},
        "synthetic_generator": {
            "identity": "synthetic-g1-v2", "namespace": "dev:",
            ...
            "positive_ids": positives, "negative_ids": negatives,
...
def _identities(bindings: dict[str, str], head: str) -> dict[str, Any]:
    return {
        ...
        "schema_sha256": bindings["schema_sha256"], "python_wheel_sha256": bindings["python_wheel"],
        "rust_wheel_sha256": bindings["rust_wheel"],
        "backend_extension_sha256": bindings["extension"],
        "encoder_directory_sha256": bindings["encoder_digest"],
        "repository_head": head, "dirty_tree_digest": bindings["dirty_tree_digest"],
        "experiment_lock_module_sha256": bindings["lock_module"],
        "gb_preflight_module_sha256": bindings["gb_module"],
    }
...
_LANE_H_MISSING_INVENTORY = [
    "historical_git_object", "historical_embedding", "historical_configuration",
    "historical_dependency_environment", "historical_ranking_output", "historical_output_root",
...
def _build_lane_c_diagnostic_manifest(workspace: Path) -> dict[str, Any]:
    ...
    def digest(component: str) -> str:
        return _measure_fixture(root, f"{component}.bin",
...
def _foreign_lanes(lane_c_manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "lane_h": el.build_foreign_lane_h_inadmissible(_LANE_H_MISSING_REASON, _LANE_H_MISSING_INVENTORY),
    }


def _lock_payload(d1_binding: dict[str, Any], d2_binding: dict[str, Any], identities: dict[str, Any],
                  bindings: dict[str, str], real: dict[str, str], foreign_lanes: dict[str, Any],
                  lane: str = "lane_p") -> dict[str, Any]:
    order = list(d1_binding["selected_record_ids"])
    order_digest = el.candidate_order_digest(order)
    schema = bindings["schema_sha256"]
    lane_identity = {
        "root": real["root"], "task_set": real["task_set"], "threshold": real["threshold"],
        "candidate_bank": real["candidate_bank"], "scorer": real["scorer"],
        "schema": schema, "provenance": order_digest,
    }
    lane_block = [lane_identity[field] for field in
                  ("root", "task_set", "threshold", "candidate_bank", "scorer", "schema", "provenance")]
    return {
        "schema_version": 2, "artifact_type": "snn-memory-experiment-lock-v2",
        "state": "fixture_only", "lane_role": lane, "seeds": list(el.SEEDS),
        "identities": identities, "d1": dict(d1_binding), "d2": dict(d2_binding),
        "scoring_target_digest": real["scorer"], "calibration_spec_digest": real["calibration_self"],
...
def _lane_real(scoring: Any, calibration: Any, bindings: dict[str, str],
               expected_task_set_digest: str) -> dict[str, str]:
    ...
    for seed in el.SEEDS:
        for condition, cue in (("trained", cues[0]), ("shuffled", cues[1])):
            tasks.append({
                ...
                "seed": seed, "lane_role": lane, "condition": condition, "cue_digest": cue,
            })
    return tasks
...
def _completeness_payload(complete: bool, cues: tuple[str, str], lane: str = "lane_p") -> dict[str, Any]:
    tasks = _tasks(cues, lane)
    ids = [task["task_id"] for task in tasks]
    payload = {
        "schema_version": 2, "artifact_type": "snn-memory-task-completeness-v2",
        "state": "fixture_only", "lane_role": lane, "seeds": list(el.SEEDS),
        "task_set_digest": task_set_digest(ids), "expected_tasks": tasks,
        "completed_task_ids": list(ids), "missing_task_ids": [], "unexpected_task_ids": [],
        "digest_failure_task_ids": [],
        "process_records": _process_records(ids),
        "completeness": True, "abort_reason": None,
    }
    if not complete:
...
def _cli(install_target: Path, arguments: list[str], expected_code: int) -> tuple[dict[str, Any], int]:
...
def _check_cli(workspace: Path, install_target: Path, record_ids: list[str], lock: Any,
               bindings: dict[str, str]) -> list[int]:
    pids: list[int] = []
    ...
    bind_inputs = ["--d1", str(workspace / "universe.json"),
                   "--cue-set", str(cue_set_dir / "cue_set.json"),
                   "--bundle-root", str(cue_set_dir)]
    ...
    report, pid = _cli(install_target, ["write", "--payload", str(payload_file),
                                        "--output", str(workspace / "cli_scoring.json")], 0)
    pids.append(pid)
    assert report["artifact_type"] == "snn-memory-scoring-target-v2"
    report, pid = _cli(install_target, ["read", "--artifact", str(workspace / "lock.json"),
                                        "--expected-type", "snn-memory-experiment-lock-v2",
                                        "--expected-sha256", lock.file_sha256], 0)
    pids.append(pid)
    report, pid = _cli(install_target, ["completeness", "--artifact", str(workspace / "complete.json")], 0)
    pids.append(pid)
    assert report["completeness"] is True
    report, pid = _cli(install_target, ["bind", "--lock", str(workspace / "lock.json"),
                                        "--scoring-target", str(workspace / "scoring.json"),
                                        "--calibration-spec", str(workspace / "calibration.json"),
                                        *bind_inputs], 0)
    pids.append(pid)
    assert report["bound"] is True
    # A CLI bind whose bundle root omits the authenticated bundles must fail, not report bound.
    _, pid = _cli(install_target, ["bind", "--lock", str(workspace / "lock.json"),
                                   "--scoring-target", str(workspace / "scoring.json"),
                                   "--calibration-spec", str(workspace / "calibration.json"),
                                   "--d1", str(workspace / "universe.json"),
                                   "--cue-set", str(cue_set_dir / "cue_set.json"),
                                   "--bundle-root", str(workspace / "no-bundles")], 2)
    pids.append(pid)
    _, pid = _cli(install_target, ["read", "--artifact", str(workspace / "lock.json"),
                                   "--expected-sha256", "0" * 64], 2)
    pids.append(pid)

    # In-process runpy for installed-module coverage of main() + handlers + error exit.
    out = _module_cli(["write", "--payload", str(payload_file),
                       "--output", str(workspace / "inproc_scoring.json")], 0)
    ...
    _module_cli(["bind", "--lock", str(workspace / "lock.json"),
                 "--scoring-target", str(workspace / "scoring.json"),
                 "--calibration-spec", str(workspace / "calibration.json"),
                 "--d1", str(workspace / "universe.json"),
                 "--cue-set", str(cue_set_dir / "cue_set.json"),
...
def _gb_child_argv(extension_sha256: str, output: Path,
                   calibration_digest: str, scoring_digest: str) -> list[str]:
    return [
        "--extension-sha256", extension_sha256, "--seed", str(GB_SEED),
        "--cue-steps", str(GB_CUE_STEPS), "--completion-steps", str(GB_COMPLETION_STEPS),
        "--n-neurons", str(GB_CONFIG.n_neurons),
        "--excitatory-fraction", str(GB_CONFIG.excitatory_fraction),
        "--connectivity", str(GB_CONFIG.connectivity), "--dt-ms", str(GB_CONFIG.dt_ms),
        "--spike-drift-ceiling", str(GB_SPIKE_CEILING),
        "--current-drift-ceiling", str(GB_CURRENT_CEILING),
        "--numerical-zero-floor", str(GB_ZERO_FLOOR), "--lane-role", "lane_p",
        "--state", "fixture_only", "--calibration-spec-digest", calibration_digest,
        "--scoring-target-digest", scoring_digest, "--output", str(output),
    ]


def _gb_child(install_target: Path, output: Path, extension_sha256: str,
              calibration_digest: str, scoring_digest: str) -> tuple[bytes, str, int]:
    environment = dict(os.environ, PYTHONPATH=str(install_target))
    argv = [sys.executable, "-m", "snn_memory.gb_preflight",
...
def _gate_lag(rows: Any, raster: list[frozenset[int]], width: int, n_neurons: int,
              lag: int, spike_ceiling: float, current_ceiling: float) -> bool:
    denominator = float(width * n_neurons)
    for tail_bin in _GATE_TAIL_BINS:
        if _gate_symmetric(raster, tail_bin, tail_bin - lag, width) / denominator > spike_ceiling:
            return False
        drift = _gate_drift(_gate_bin_vector(rows, tail_bin, width),
                            _gate_bin_vector(rows, tail_bin - lag, width))
        if drift > current_ceiling:
            return False
    return True


def _gate_class(rows: Any, raster: list[frozenset[int]], width: int, n_neurons: int,
                spike_ceiling: float, current_ceiling: float, zero_floor: float) -> str:
    silent = True
...
def _check_gb_two_child(workspace: Path, install_target: Path, extension_sha256: str,
                        calibration_digest: str, scoring_digest: str) -> dict[str, Any]:
    ...
    assert struct.pack(">d", energy).hex() == first_bits, "independent U-2 fold differs from evidence"
    ...
    assert _gate_class(rows, raster, width, GB_CONFIG.n_neurons, GB_SPIKE_CEILING,
                       GB_CURRENT_CEILING, GB_ZERO_FLOOR) == evidence.payload["trajectory_class"]
...
def _check_gb_negatives(extension_sha256: str) -> None:
    ...
    def evaluate(**changes: Any) -> Callable[[], object]:
        parameters: dict[str, Any] = {
            "completion_steps": 32, "spike_drift_ceiling": 0.05,
            "current_drift_ceiling": 0.05, "numerical_zero_floor": 1e-09,
        }
    ...
    _expect("completion phase length differs",
            lambda: evaluate_gb_descriptor(backend, inputs, 8, config, completion_steps=40,
                                           spike_drift_ceiling=0.05, current_drift_ceiling=0.05,
                                           numerical_zero_floor=1e-09))
    _expect("verified StreamBackend is required",
            lambda: evaluate_gb_descriptor(_not_a_backend(), inputs, 8, config, completion_steps=32,
                                           spike_drift_ceiling=0.05, current_drift_ceiling=0.05,
                                           numerical_zero_floor=1e-09))
    ...
    _expect("external L1 must be exactly zero at every completion row",
            lambda: evaluate_gb_descriptor(backend, replace(active, packets=packets), 8, config,
                                           completion_steps=32, spike_drift_ceiling=0.05,
                                           current_drift_ceiling=0.05, numerical_zero_floor=1e-09))
...
def _check_negatives(workspace: Path, d1_binding: dict[str, Any], d2_binding: dict[str, Any],
                     record_ids: list[str], scoring: Any, calibration: Any,
                     identities: dict[str, Any], lock: Any, d1_art: Any, d2_art: Any,
                     bindings: dict[str, str], locked_identities: list[str], foreign_d1: Any,
                     expected_task_set_digest: str, completeness: Any,
                     cues: tuple[str, str], real: dict[str, str], bundle_reps: dict[str, str],
                     bundle_arts: list[Any], foreign_lanes: dict[str, Any],
                     lane_c_manifest: dict[str, Any], lane_h_manifest: dict[str, Any]) -> None:
    ...
    _expect("SHA-256 mismatch", lambda: validate_artifact_bytes(good, expected_file_sha256="0" * 64))
    _expect("duplicate JSON key",
            lambda: validate_artifact_bytes(b'{"artifact_type":"x","artifact_type":"y"}'))
    _expect("non-finite JSON constant", lambda: validate_artifact_bytes(b'{"schema_version":Infinity}'))
    _expect("contains a non-finite JSON number", lambda: validate_artifact_bytes(b'{"schema_version":1e400}'))
    ...
    _expect("unknown or missing artifact_type", lambda: validate_artifact_bytes(b'{"schema_version":2}'))
    _expect("cannot seal an unknown artifact_type",
            lambda: el.seal_payload({"artifact_type": "snn-memory-bogus-v2"}))
    _expect("differs from the expected type",
    ...
    _expect("schema validation failed",
    ...
    _check_lock_binding_negatives(d1_binding, d2_binding, scoring, calibration, identities,
                                  bindings, real, d1_art, d2_art, lock, foreign_d1, foreign_lanes,
                                  lane_c_manifest, bundle_arts)
...
def _check_scoring_negatives(record_ids: list[str], bindings: dict[str, str]) -> None:
    ...
    assert validate_artifact_bytes(el.seal_payload(base)).artifact_type == "snn-memory-scoring-target-v2"

    def unsorted(payload: dict[str, Any]) -> None:
        payload["candidate_order"] = list(reversed(payload["candidate_order"]))

    _expect("must be lexical immutable-record-ID order",
    ...
    _expect("one candidate signature digest is required per candidate",
    ...
    _expect("candidate-bank digest does not bind the candidate bank",
    ...
    _expect("scorer digest does not bind the scorer configuration",
...
def _check_calibration_negatives(bindings: dict[str, str]) -> None:
    ...
    assert validate_artifact_bytes(el.seal_payload(base)).artifact_type == "snn-memory-gb-calibration-spec-v2"

    def negative_grid(payload: dict[str, Any]) -> None:
        payload["spike_drift_ceiling_grid"] = [-0.1]

    _expect("must contain only finite non-negative values",
    ...
    _expect("synthetic positive IDs are not the canonical per-seed corpus",
    ...
    _expect("synthetic negative IDs are not the matched four-family corpus",
    ...
    _expect("synthetic validation partition digest is not bound",
    ...
    _expect("synthetic config digest does not bind the generator configuration",
    ...
    _expect("synthetic selection partition digest is not bound",
    ...
    _expect("calibration task-set digest does not bind the synthetic inventory",
            lambda: validate_artifact_bytes(_craft(base, break_task_set)))


def _check_lock_binding_negatives(d1_binding: dict[str, Any], d2_binding: dict[str, Any],
                                  scoring: Any, calibration: Any, identities: dict[str, Any],
                                  bindings: dict[str, str], real: dict[str, str],
                                  d1_art: Any, d2_art: Any, lock: Any, foreign_d1: Any,
                                  foreign_lanes: dict[str, Any], lane_c_manifest: dict[str, Any],
                                  bundle_arts: list[Any]) -> None:
    def build(lane: str) -> dict[str, Any]:
        return _lock_payload(d1_binding, d2_binding, identities, bindings, real, foreign_lanes, lane=lane)
    ...
    _expect("D2 source-universe binding differs from the D1 binding",
    ...
    _expect("lock repository HEAD differs from the D1 binding",
            lambda: validate_artifact_bytes(_craft(base, break_head)))

    def break_candidate(payload: dict[str, Any]) -> None:
        payload["candidate_order"] = list(reversed(payload["candidate_order"]))

    _expect("candidate order differs from the D1 selected record IDs",
    ...
    _expect("lock candidate-order digest is not bound",
    ...
    _expect("lock lane-role digest is not bound",
    ...
    _expect("lock lane-domain digest is not bound",
    ...
    top_level = {"root": "output_root_digest", "task_set": "expected_task_set_digest",
                 "scorer": "scoring_target_digest"}
    for field in _LANE_DISTINGUISHING:
        foreign_value = foreign_identity[field]

        def contaminate(payload: dict[str, Any], key: str = field, value: str = foreign_value) -> None:
        ...
        _expect("a foreign-lane identity contaminates a Lane-P identity",
    ...
    _expect("only a Lane-P lock is admissible",
            lambda: bind(lane_c_role, scoring, calibration))
    lane_h_role = validate_artifact_bytes(el.seal_payload(build("lane_h")))
    _expect("only a Lane-P lock is admissible",
            lambda: bind(lane_h_role, scoring, calibration))
    _expect("requires the exact lock/scoring/calibration types",
    ...
    contaminated_foreign = {"lane_c": el.build_foreign_lane_c(contaminated_manifest),
                            "lane_h": foreign_lanes["lane_h"]}
    contaminated_lock = validate_artifact_bytes(el.seal_payload(
        _lock_payload(d1_binding, d2_binding, identities, bindings, real, contaminated_foreign)))
    _expect("a foreign-lane identity contaminates a bound Lane-P artifact identity",
    ...
    def bind_scoring(payload: dict[str, Any], scoring_art: Any) -> None:
        ...
        payload["lane_identity"]["candidate_bank"] = \
            scoring_art.payload["identities"]["candidate_bank_digest"]
        payload["lane_domain_digest"] = el.lane_domain_digest("lane_p", el.lane_identity_fields(payload))
    ...
    _expect("scoring-target lane role contaminates the Lane-P lock",
    ...
    _expect("scoring-target identity differs from the lock identity block",
    ...
    _expect("lock calibration digest does not bind the calibration spec",
    ...
    _expect("threshold identity does not bind the calibration thresholds",
    ...
    scoring_short = validate_artifact_bytes(el.seal_payload(_scoring_target(record_order[:-1], bindings)))
    lock_binds_short = validate_artifact_bytes(
        _craft(base, lambda p: bind_scoring(p, scoring_short)))
    _expect("scoring candidate order must be the lexical Lane-P candidate set",
    ...
    _expect("calibration-spec lane role contaminates the Lane-P lock",
    ...
    lock_binds_bad_cal = validate_artifact_bytes(
        _craft(base, lambda p: p.update(calibration_spec_digest=calibration_bad.payload_self_sha256)))
    _expect("calibration identity differs from the lock identity block",
    ...
    _expect("candidate-bank identity does not bind the scoring candidate bank",
    ...
    _expect("lock D2 binding differs from the authenticated cue set",
    ...
    _expect("lock D1 binding differs from the authenticated D1 artifact",
    ...
    _expect("scoring-target digest does not bind",
            lambda: bind(lock, mutated_art, calibration))


def _check_foreign_lane_negatives(base: dict[str, Any], lane_c_manifest: dict[str, Any],
                                  lane_h_manifest: dict[str, Any]) -> None:
    good = el.seal_payload(base)
    ...
    _expect("foreign Lane-C identity is not derived from its manifest",
    ...
    _expect("foreign Lane-C evidence self digest mismatch",
    ...
    _expect("schema validation failed", lambda: validate_artifact_bytes(_craft(base, role_change_c)))
    ...
    _expect("foreign Lane-H evidence self digest mismatch",
    ...
    def synthetic_h_identity(payload: dict[str, Any]) -> None:
        zero_identity = {key: "0" * 64 for key in
                         ("root", "task_set", "threshold", "candidate_bank", "scorer", "schema", "provenance")}
        block = {"lane_role": "lane_h", "status": "admissible_authenticated_historical_evidence",
                 "manifest": dict(lane_h_manifest), "identity": zero_identity,
                 "self_sha256": "0" * 64}
        payload["foreign_lanes"]["lane_h"] = block

    _expect("admissible Lane-H identity is not derived from its manifest",
    ...
    def status_substitution(payload: dict[str, Any]) -> None:
        payload["foreign_lanes"]["lane_h"]["status"] = "admissible_authenticated_historical_evidence"
    ...
    def partial_h(payload: dict[str, Any]) -> None:
        ...
        block = {"lane_role": "lane_h", "status": "admissible_authenticated_historical_evidence",
                 "manifest": manifest, "identity": el.lane_h_manifest_identity(lane_h_manifest),
                 "self_sha256": "0" * 64}
        payload["foreign_lanes"]["lane_h"] = block
    ...
    def inject_raw_h_component(payload: dict[str, Any]) -> None:
        ...
        payload["identities"]["backend_extension_sha256"] = lane_h_manifest["historical_embedding_sha256"]
    ...
    assert admissible_lock.payload["foreign_lanes"]["lane_h"]["status"] == \
...
def _check_completeness_negatives(cues: tuple[str, str]) -> None:
    ...
    _expect("completeness flag disagrees",
            lambda: validate_artifact_bytes(_craft(good, missing_seed)))
    ...
    _expect("task ID does not bind its full task identity",
    ...
    def multiset_imbalance(payload: dict[str, Any]) -> None:
        ...
        payload["expected_tasks"][0]["task_id"] = task_identity_digest(
            payload["expected_tasks"][0]["seed"], "lane_p", "zero_recurrence",
    ...
    def process_reuse(payload: dict[str, Any]) -> None:
        payload["process_records"][1]["process_instance"] = payload["process_records"][0]["process_instance"]

    _expect("completeness flag disagrees",
            lambda: validate_artifact_bytes(_craft(good, process_reuse)))
    ...
    _expect("expected task lane role differs",
            lambda: validate_artifact_bytes(_craft(good, wrong_lane)))

    def out_of_set_seed(payload: dict[str, Any]) -> None:
        payload["expected_tasks"][0]["seed"] = 999
        payload["expected_tasks"][0]["task_id"] = task_identity_digest(
            999, "lane_p", payload["expected_tasks"][0]["condition"],
            payload["expected_tasks"][0]["cue_digest"])
        ids = [task["task_id"] for task in payload["expected_tasks"]]
        payload["task_set_digest"] = task_set_digest(ids)
        payload["completed_task_ids"] = ids
    ...
    _expect("outside the frozen seed set",
    ...
    _expect("missing task set is inconsistent",
    ...
    _expect("unexpected tasks overlap",
    ...
    _expect("digest-failure tasks are not expected",
    ...
    _expect("must not declare an abort reason",
    ...
    _expect("requires a known abort reason",
            lambda: validate_artifact_bytes(_craft(good, bad_abort)))
...
def _check_completeness_binding_negatives(lock: Any, completeness: Any, cues: tuple[str, str]) -> None:
    _expect("requires the exact lock/completeness types",
            lambda: bind_task_completeness(lock, lock))
    lane_c_completeness = validate_artifact_bytes(
        el.seal_payload(_completeness_payload(True, cues, lane="lane_c")))
    _expect("completeness lane role differs from the lock lane",
    ...
    _expect("completeness task-set digest differs from the lock expected task set",
            lambda: bind_task_completeness(lock, other_completeness))


def _gb_evidence_payload(extension_sha256: str, workspace: Path, identities: dict[str, Any]) -> dict[str, Any]:
    backend = load_stream_backend(BackendIdentity(2, "0.1.0", extension_sha256))
    inputs = gb.build_episode(GB_CONFIG, GB_SEED, GB_CUE_STEPS, GB_COMPLETION_STEPS)
    descriptor = evaluate_gb_descriptor(
        backend, inputs, GB_CUE_STEPS, GB_CONFIG, completion_steps=GB_COMPLETION_STEPS,
        spike_drift_ceiling=GB_SPIKE_CEILING, current_drift_ceiling=GB_CURRENT_CEILING,
        numerical_zero_floor=GB_ZERO_FLOOR)
    del identities
    return gb.descriptor_payload(descriptor, lane_role="lane_p", state="fixture_only",
                                 calibration_spec_digest=SHA, scoring_target_digest=SHA)
...
def _check_gb_evidence_negatives(workspace: Path, identities: dict[str, Any]) -> None:
    ...
    assert validate_artifact_bytes(el.seal_payload(base)).artifact_type == "snn-memory-gb-preflight-evidence-v2"

    def wrong_energy_bits(payload: dict[str, Any]) -> None:
        payload["recurrent_energy_bits"] = "0000000000000000"

    _expect("energy bits differ from the recomputed energy",
    ...
    _expect("settled flag disagrees with the trajectory class",
    ...
    _expect("completion rows differ from completion_steps",
    ...
    _expect("completion timesteps are not contiguous ascending",
    ...
    _expect("schema validation failed",
            lambda: validate_artifact_bytes(_craft(base, changed_epsilon)))

    def mutate_row(payload: dict[str, Any]) -> None:
        payload["completion_rows"][10]["net_l2"] = payload["completion_rows"][10]["net_l2"] + 1.0

    _expect("recurrent energy differs from the recomputed fold",
    ...
    _expect("spike raster is not ascending",
    ...
    _expect("spike index exceeds the population",
    ...
    _expect("spike raster differs from completion_steps",
    ...
    _expect("half-life differs from the recomputed decay",
    ...
    _expect("tail Hamming differs from the recomputed raster",
    ...
    def wrong_trajectory(payload: dict[str, Any]) -> None:
        payload["trajectory_class"] = (
            "wandering_active" if payload["trajectory_class"] != "wandering_active" else "silent_decay")

    _expect("trajectory class differs from the recomputed dynamics",
    ...
    _expect("net-L2 must be finite and non-negative",
    ...
    _expect("recurrent energy is not finite non-negative",
            lambda: validate_artifact_bytes(_craft(base, overflow_net_l2)))


def _check_dev_namespace(record_ids: list[str], locked_identities: list[str],
                         bundle_reps: dict[str, str]) -> None:
    require_dev_namespace_disjoint(["dev:mem-0", "dev:mem-1"], record_ids, locked_identities)
    _expect("development IDs must use the dev: namespace",
            lambda: require_dev_namespace_disjoint(["mem-0"], record_ids, locked_identities))
    _expect("development and immutable record identities overlap",
    ...
    for representative in bundle_reps.values():
        _expect("development identity overlaps a locked D2 identity",
...
def _check_path_negatives(workspace: Path, record_ids: list[str], bindings: dict[str, str]) -> None:
    ...
    _expect("output parent must be an absolute canonical path",
            lambda: write_artifact(_scoring_target(record_ids, bindings), parent_link / "new.json"))
    _expect("no-clobber", lambda: write_artifact(_scoring_target(record_ids, bindings),
                                                 workspace / "lock.json"))
...
def _dirty_tree_digest(repo: Path) -> str:
    ...
    return _sha(b"remanentia:snn-v2-dirty-tree:v1\0" + len(status).to_bytes(8, "big") + status + diff)
...
def _locked_d2_inventory(d1_art: Any, d2_art: Any, bundle_arts: list[Any]) -> list[str]:
    ...
    inventory.update([d1_art.file_sha256, str(d1_art.payload_self_sha256), str(d1["repository"]["head"])])
    ...
    for record in d1["selected"]:
        inventory.update([str(record["path"]), str(record["normalized_path"]),
                          str(record["blob_oid"]), str(record["record_id"]),
    ...
    inventory.add(_sha(b"remanentia:snn-v2-derivation:v1\0"
    ...
    for record in d2["records"]:
        ...
        inventory.update(str(calibration[key]) for key in ("cue_id", "path", "sha256", "normalized_text_sha256"))
        for cue_base in record["evaluation_base_cues"]:
            inventory.add(str(cue_base["cue_id"]))
            for variant in cue_base["variants"]:
                inventory.update(str(variant[key]) for key in
                                 ("variant_id", "path", "sha256", "normalized_text_sha256", "tokenizer_digest"))
    ...
    for bundle_art in bundle_arts:
        ...
        inventory.update(str(payload[key]) for key in
                         ("cue_id", "text_sha256", "normalized_text_sha256", "self_sha256"))
        inventory.add(_embedding_digest(bundle_art))
        inventory.update(str(payload["encoder"][key]) for key in
                         ("identity", "directory_sha256", "config_digest"))
...
_ACTIVE_CONFIGS = (
    ...
    ModelConfig(n_neurons=64, excitatory_fraction=0.95, connectivity=1.0, dt_ms=1.0,
                v_threshold_mv=-64.5, weight_max=50.0, refractory_ms=0.0, tau_m_ms=40.0),
    ...
    ModelConfig(n_neurons=64, excitatory_fraction=0.95, connectivity=1.0, dt_ms=1.0,
                v_threshold_mv=-64.5, weight_max=50.0, refractory_ms=7.0, tau_m_ms=40.0),
    ModelConfig(n_neurons=128, excitatory_fraction=0.9, connectivity=1.0, dt_ms=1.0,
                v_threshold_mv=-64.5, weight_max=50.0, refractory_ms=1.0, tau_m_ms=40.0),
)


def _process_episode(backend: Any, config: ModelConfig, seed: int, workspace: Path,
                     label: str) -> tuple[str, int | None]:
    """Seal, re-read, and independently gate-recompute one real episode's descriptor."""
    inputs = gb.build_episode(config, seed, 8, 32)
    descriptor = evaluate_gb_descriptor(
        backend, inputs, 8, config, completion_steps=32, spike_drift_ceiling=0.05,
        current_drift_ceiling=0.05, numerical_zero_floor=1e-09)
    output = workspace / f"gb_{label}.json"
    write_artifact(gb.descriptor_payload(descriptor, lane_role="lane_p", state="fixture_only",
                                         calibration_spec_digest=SHA, scoring_target_digest=SHA), output)
    ...
    assert _gate_class(rows, raster, 4, config.n_neurons, 0.05, 0.05, 1e-09) == \
        evidence.payload["trajectory_class"]
    return str(descriptor.trajectory_class), descriptor.half_life_steps


def _check_active_episode(workspace: Path, install_target: Path, extension_sha256: str) -> list[str]:
    ...
    _quiescent_class, quiescent_half_life = _process_episode(
        backend, ModelConfig(n_neurons=64, excitatory_fraction=0.8, connectivity=0.5, dt_ms=1.0,
                             v_threshold_mv=1000.0), 3, workspace, "quiescent")
    ...
    for index, config in enumerate(_ACTIVE_CONFIGS):
        for seed in (3, 7):
            trajectory, _half_life = _process_episode(backend, config, seed, workspace,
                                                      f"active_{index}_{seed}")
...
def _witness_search(workspace: Path, extension_sha256: str) -> dict[str, Any]:
    ...
    for refractory in _WITNESS_REFRACTORY:
        for weight in _WITNESS_WEIGHT:
            for seed in _WITNESS_SEEDS:
                config = ModelConfig(n_neurons=64, excitatory_fraction=0.95, connectivity=1.0,
                                     dt_ms=1.0, v_threshold_mv=-64.5, weight_max=weight,
                                     refractory_ms=refractory, tau_m_ms=40.0)
                episodes += 1
                inputs = gb.build_episode(config, seed, 8, 32)
                result = backend.run(inputs, 8, False, config)
                rows = _gate_rows(result, 8, 32)
    ...
    return {
        "grid": {
            "refractory_ms": list(_WITNESS_REFRACTORY), "weight_max": list(_WITNESS_WEIGHT),
            "seeds": list(_WITNESS_SEEDS), "n_neurons": 64, "connectivity": 1.0,
            "excitatory_fraction": 0.95, "v_threshold_mv": -64.5,
        },
...
def _check_gb_cli_in_process(workspace: Path, extension_sha256: str) -> None:
    ...
    def run(argv: list[str], expected_code: int) -> str:
        ...
        try:
            ...
            assert exit_info.code == expected_code, f"gb CLI exit {exit_info.code}: {stderr.getvalue()}"
        finally:
            sys.argv = previous
        return stdout.getvalue()
...
def main() -> int:
    ...
    assert arguments.public_license.read_bytes() == packaged_schema.with_name(
        packaged_schema.name + ".license").read_bytes()
    ...
    scoring = write_artifact(_scoring_target(record_ids, bindings), arguments.workspace / "scoring.json")
    calibration = write_artifact(_calibration_spec(bindings), arguments.workspace / "calibration.json")
    ...
    completeness = write_artifact(_completeness_payload(True, cues), arguments.workspace / "complete.json")
    bind_lane_isolation(lock, scoring, calibration, d1_art, d2_art, bundle_arts)
    bind_d1_d2(lock, d1_art, d2_art)
    bind_task_completeness(lock, completeness)

    foreign_d1 = _foreign_d1(arguments.workspace)
    foreign_bundle = _foreign_bundle(arguments.workspace, arguments.encoder_checkpoint,
                                     arguments.encoder_digest)
    _check_bundle_completeness_negatives(lock, scoring, calibration, d1_art, d2_art, bundle_arts,
                                         foreign_bundle)
    ...
    gb_evidence = _check_gb_two_child(arguments.workspace, install_target, arguments.extension_sha256,
                                      calibration.payload_self_sha256, scoring.payload_self_sha256)
    active_class = _check_active_episode(arguments.workspace, install_target, arguments.extension_sha256)
    ...
    _check_negatives(arguments.workspace, d1_binding, d2_binding, record_ids, scoring, calibration,
                     identities, lock, d1_art, d2_art, bindings, locked_identities, foreign_d1,
                     expected_task_set_digest, completeness, cues, real, bundle_reps,
                     bundle_arts, foreign_lanes, lane_c_manifest, lane_h_manifest)
