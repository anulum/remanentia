...
static RE_FILE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"[\w/\\]+\.(?:py|rs|md|json|yaml|toml)\b").unwrap());
...
fn regex_entities(text: &str) -> Vec<(String, String, f64)> {
    ...
    let pats: &[(&str, &[&str])] = &[
        ("project", &["director-ai","sc-neurocore","scpn-control","scpn-fusion","scpn-phase-orchestrator","scpn-quantum-control","remanentia"]),
        ("algorithm", &["stdp","lif","kuramoto","hopfield","bcpnn","csdp","tf-idf","bm25","stuart-landau"]),
        ("hardware", &["gpu","cuda","loihi","gtx 1060"]),
        ("software tool", &["pytorch","numpy","scipy","fastapi","docker"]),
    ];
    for (label, terms) in pats {
        for t in *terms {
            if lo.contains(t) { out.push((t.to_string(), label.to_string(), 0.5)); }
        }
    }
    for m in RE_VER.find_iter(text) { out.push((m.as_str().into(), "version number".into(), 0.6)); }
    for m in RE_FILE.find_iter(text) {
        let n = m.as_str().rsplit('/').next().unwrap_or(m.as_str());
        let n = n.rsplit('\\').next().unwrap_or(n);
        if n.len() > 3 { out.push((n.into(), "file path".into(), 0.5)); }
    }
    out
}
...
#[pyfunction]
fn extract_relations(text: &str, entity_texts: Vec<String>) -> Vec<(String, String, String, String)> {
    let lo = text.to_lowercase();
        ...
        for j in (i+1)..entity_texts.len() {
            ...
            for (pat, rt) in RELATION_PATTERNS.iter() {
                if pat.is_match(between) {
                    let ev = if between.len() > 200 { &between[..200] } else { between };
