...
# Runtime and build toolchain install from a fully hash-pinned lock
# (requirements/docker.txt, generated by pip-compile --generate-hashes).
# --require-hashes rejects any artifact whose hash is not pinned, so the build
# cannot silently float a transitive dependency. The local package then
# installs with --no-build-isolation against the pinned setuptools/wheel, so
# there is no unpinned build-time fetch either.
RUN python -m pip install --no-cache-dir --require-hashes -r requirements/docker.txt \
