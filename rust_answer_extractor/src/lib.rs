...
static RE_DATE_ISO: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"\b\d{4}-\d{2}-\d{2}\b").unwrap());
static RE_DATE_WRITTEN: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?:January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{1,2}(?:,?\s*\d{4})?").unwrap());
...
static RE_NUMBER: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"\b(\d[\d,]*(?:\.\d+)?)\b").unwrap());
static RE_NAME: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"[A-Z][a-z]+(?:\s+[A-Z][a-z]+)+").unwrap());
static RE_SINGLE_NAME: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"\b[A-Z][a-z]{2,}\b").unwrap());
static RE_WORD3: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"\w{3,}").unwrap());
static RE_WHEN: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?i)\bwhen\b|\bwhat date\b|\bwhat time\b").unwrap());
static RE_HOW_MANY: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?i)\bhow many\b|\bhow much\b|\bcount\b|\bnumber of\b").unwrap());
static RE_VERSION_Q: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?i)\bversion\b|\brelease\b|\bv\d").unwrap());
static RE_WHO: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?i)\bwho\b|\bwhose\b|\bwhom\b").unwrap());
static RE_YESNO: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?i)^(is|are|was|were|did|does|do|has|have|can|will|should)\b").unwrap());
static RE_PERCENT_Q: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?i)\bpercent\b|\baccuracy\b|\bscore\b|\brate\b|\b%").unwrap());
...
fn tokenise(q: &str) -> HashSet<String> {
    RE_WORD3.find_iter(&q.to_lowercase()).map(|m| m.as_str().to_string()).collect()
}
...
fn best_prox(cands: &[(String, usize)], text: &str, query: &str) -> String {
    ...
    if cands.len() == 1 || query.is_empty() { return cands[0].0.clone(); }
    let qt = tokenise(query);
    cands.iter().max_by_key(|(v, p)| proximity(text, *p, v.len(), &qt)).unwrap().0.clone()
}

fn extract_date(text: &str, query: &str) -> Option<String> {
    let mut c: Vec<(String, usize)> = Vec::new();
    for m in RE_DATE_ISO.find_iter(text) { c.push((m.as_str().into(), m.start())); }
    for m in RE_DATE_WRITTEN.find_iter(text) { c.push((m.as_str().into(), m.start())); }
    if c.is_empty() { None } else { Some(best_prox(&c, text, query)) }
}

fn extract_number(text: &str, query: &str) -> Option<String> {
    let c: Vec<(String, usize)> = RE_NUMBER.find_iter(text)
        .filter(|m| !RE_YEAR_ONLY.is_match(m.as_str()) && !m.as_str().is_empty())
        .map(|m| (m.as_str().into(), m.start())).collect();
    if c.is_empty() { None } else { Some(best_prox(&c, text, query)) }
}

fn extract_version(t: &str) -> Option<String> { RE_VERSION.find(t).map(|m| m.as_str().into()) }
fn extract_pct(t: &str) -> Option<String> { RE_PERCENTAGE.find(t).map(|m| m.as_str().into()) }

fn extract_name(text: &str, query: &str) -> Option<String> {
    let mut c: Vec<(String, usize)> = RE_NAME.find_iter(text).map(|m| (m.as_str().into(), m.start())).collect();
    if c.is_empty() {
        let qt = tokenise(query);
        for m in RE_SINGLE_NAME.find_iter(text) {
            ...
            let w = safe_window(text, s, e).to_lowercase();
            if qt.iter().any(|t| w.contains(t.as_str())) { c.push((m.as_str().into(), m.start())); }
        }
    }
    if c.is_empty() { None } else { Some(best_prox(&c, text, query)) }
}
...
fn extract_yesno(text: &str, query: &str) -> String {
    ...
    let neg: &[&str] = &["not ","no ","never ","doesn't ","didn't ","isn't ","wasn't ","weren't ","won't ","wouldn't ","couldn't ","shouldn't ","haven't ","hasn't ","can't ","cannot ","unable ","failed to ","stopped ","quit ","gave up "];
    let qt = tokenise(query);
    let (mut nh, mut ph) = (0i32, 0i32);
    for tok in &qt {
        if let Some(pos) = t.find(tok.as_str()) {
            let s = pos.saturating_sub(40); let e = (pos+40).min(t.len());
            let w = safe_window(&t, s, e);
            if neg.iter().any(|n| w.contains(n)) { nh += 1; } else { ph += 1; }
        }
    }
    if nh > ph { "No".into() } else { "Yes".into() }
}

#[pyfunction]
fn extract_answer(query: &str, paragraph: &str) -> Option<String> {
    let q = query.to_lowercase();
    if RE_WHEN.is_match(&q) { return extract_date(paragraph, query); }
    if RE_HOW_MANY.is_match(&q) { return extract_number(paragraph, query); }
    if RE_VERSION_Q.is_match(&q) { return extract_version(paragraph); }
    if RE_WHO.is_match(&q) { return extract_name(paragraph, &q); }
    if RE_YESNO.is_match(&q) { return Some(extract_yesno(paragraph, query)); }
    if RE_PERCENT_Q.is_match(&q) { return extract_pct(paragraph); }
    extract_pct(paragraph).or_else(|| extract_version(paragraph)).or_else(|| extract_date(paragraph, "")).or_else(|| extract_number(paragraph, ""))
}
...
fn split_sents(text: &str) -> Vec<&str> {
    ...
    if start < text.len() { sents.push(&text[start..]); }
    sents
}
...
fn extract_best_sentence(query: &str, paragraph: &str) -> Option<String> {
    ...
    let qt = tokenise(query);
    sents.iter().max_by_key(|s| {
        let st: HashSet<String> = RE_WORD3.find_iter(&s.to_lowercase()).map(|m| m.as_str().into()).collect();
...
fn fuzzy_match(candidate: &str, gold: &str, threshold: f64) -> bool {
    ...
    if c.is_empty() || g.is_empty() { return false; }
    if c == g || c.contains(&g) || g.contains(&c) { return true; }
...
fn normalize_number(text: &str) -> Option<String> {
    ...
    if let Some(c) = RE_NUM.captures(&t) { return Some(c[1].replace(',', "")); }
    let words: std::collections::HashMap<&str, u64> = [("zero",0),("one",1),("two",2),("three",3),("four",4),("five",5),("six",6),("seven",7),("eight",8),("nine",9),("ten",10),("eleven",11),("twelve",12),("thirteen",13),("fourteen",14),("fifteen",15),("sixteen",16),("seventeen",17),("eighteen",18),("nineteen",19),("twenty",20),("thirty",30),("forty",40),("fifty",50),("sixty",60),("seventy",70),("eighty",80),("ninety",90),("hundred",100),("thousand",1000)].into_iter().collect();
    let mut cur = 0u64; let mut found = false;
    for p in t.split(|c: char| c == '-' || c.is_whitespace()).filter(|s| !s.is_empty()) {
        if let Some(&v) = words.get(p) { if v >= 100 { cur = cur.max(1)*v; } else { cur += v; } found = true; }
        else if p == "and" { continue; } else { break; }
    }
    if found { Some(cur.to_string()) } else { None }
}
...
mod tests {
    ...
    fn extract_yesno_handles_multibyte() {
        let text = "The answer is no, ¥500 tickets are not available on the Tokyo 東京 subway today";
