...
# Remanentia — Experiment 6: Full pipeline
...
def hybrid_search(query, turns, para_tokens, idf, avg_dl, turn_embs, top_k=20):
    """BM25(0.4) + embedding(0.6) + entity boost + cross-encoder rerank."""
    n = len(turns)
    if n == 0:
        return []
...
def llm_answer(question, turns, retrieved_indices):
    if not client:
        return None
    context = "\n\n".join(
        f"[Turn {idx}]: {turns[idx][:500]}" for idx, _ in retrieved_indices[:10] if idx < len(turns)
    )
...
for ci, conv in enumerate(ds):
    ...
    if (ci + 1) % 5 == 0:
        ...
        Path("exp6_progress.txt").write_text(
...
out = {
    "experiment": "Full pipeline: hybrid+CE+temporal+LLM",
...
Path("exp6_results.json").write_text(json.dumps(out, indent=2))
